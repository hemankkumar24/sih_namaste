// FILE: server/src/controllers/doctorController.js
...
// ... (getProfile, updateProfile, getDashboardData, uploadLegacyPrescription, findPatient functions remain unchanged)
// --- Profile Management ---
...
    const { identifier } = req.query; // Removed type as we search by ABHA only now
    if (!identifier) {
...
        });
    } catch (error) {
        logger.error(error, `Failed to find patient by abha: ${identifier}`);
        next(error);
    }
};

const createConsultation = async (req, res, next) => {
    const { patientIdentifier, vitals, diagnoses, medications, notes } = req.body;
    const doctorId = req.user.id;

    try {
        const patient = await prisma.patientProfile.findUnique({
            where: { abhaNumber: patientIdentifier },
            include: { user: true }
        });
        if (!patient) {
            return res.status(404).json({ message: `Patient with ABHA number ${patientIdentifier} not found.` });
        }
